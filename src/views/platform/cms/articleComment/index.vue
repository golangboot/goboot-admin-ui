...
<script>
	...
	export default {
		name: 'platformCmsArticleComment',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.platform.cms.articleComment.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.cms.articleComment.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.cms.articleComment.update.put(reqData);
