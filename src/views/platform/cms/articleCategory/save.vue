...
<script>
	export default {
		...
		methods: {
			...
			async getTreeList(){
				var res = await this.$API.platform.cms.articleCategory.tree.get();
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						var res;
						if (this.form.id) {
							res = await this.$API.platform.cms.articleCategory.update.put(this.form)
						} else {
							res = await this.$API.platform.cms.articleCategory.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.cms.articleCategory.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
			}
