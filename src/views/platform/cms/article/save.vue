					...
					<el-row :gutter="20">
						<el-col :span="12">
							<el-form-item label="封面图片" prop="image">
								<sc-upload :width="80" :height="80" v-model="form.image" title="请上传封面图片"></sc-upload>
							</el-form-item>
						</el-col>
					</el-row>
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						var res;
						if (this.form.id) {
							res = await this.$API.platform.cms.article.update.put(this.form)
						} else {
							res = await this.$API.platform.cms.article.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.cms.article.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
				this.loading = false
