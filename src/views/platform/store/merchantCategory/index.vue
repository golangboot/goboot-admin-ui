	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="经营类目名称" prop="name" width="150"></el-table-column>
        <el-table-column label="经营类目要求" prop="description" width="300">
        ...
        <el-table-column label="手续费" prop="transactionFeeRate" header-align="center" align="center" min-width="120" sortable>
          <template #default="scope">
            <span>{{ scope.row.transactionFeeRate || 0 }}%</span>
          </template>
        </el-table-column>
...
<script>
	...
	export default {
		name: 'platformStoreMerchantCategory',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.platform.store.merchantCategory.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.merchantCategory.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.merchantCategory.update.put(reqData);
