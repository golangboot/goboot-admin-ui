	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.store.category.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.category.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.category.update.put(reqData);
			...
			async treeNodeDrop(draggingNode, dropNode, dropType) {
				treeUtils.treeNodeDrop(draggingNode, dropNode, dropType, async data => {
					this.loading = true;
					let res = data.id
						? await this.$API.platform.store.category.update.put(data)
						: await this.$API.platform.store.category.add.post(data);
					if (res.code == 200) {
