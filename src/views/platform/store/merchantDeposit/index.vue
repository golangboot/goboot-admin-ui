...
<script>
	...
	export default {
		name: 'platformStoreMerchantDeposit',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.platform.store.merchantDeposit.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.merchantDeposit.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.merchantDeposit.update.put(reqData);
