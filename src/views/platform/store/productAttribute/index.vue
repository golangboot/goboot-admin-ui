	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.store.productAttribute.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.productAttribute.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.productAttribute.update.put(reqData);
