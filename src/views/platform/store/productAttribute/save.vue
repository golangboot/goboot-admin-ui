		...
		<el-form :model="form" :rules="rules" :disabled="mode=='show'" ref="dialogForm" label-width="150px" label-position="right">
			...
			<el-form-item label="所属商品规格" prop="productSpecId">
				<select-remote v-model="form.productSpecId"
							   :apiObj="$API.platform.store.productSpec.list"
		...
		methods: {
				...
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						if (this.form.id) {
							...
							res = await this.$API.platform.store.productAttribute.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.store.productAttribute.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
