		...
		<el-container class="is-vertical">
			...
			<el-main class="nopadding">
				<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
					...
					<el-table-column label="关键字名称" prop="name" width="150" :show-overflow-tooltip="true"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.store.searchKeyword.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.searchKeyword.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.searchKeyword.update.put(reqData);
