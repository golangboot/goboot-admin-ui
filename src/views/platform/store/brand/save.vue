			...
			<el-row :gutter="20">
				...
				<el-col :span="12">
					<el-form-item label="品牌图片" prop="image">
						<sc-upload :width="80" :height="80" v-model="form.image" title="请上传品牌图片"></sc-upload>
					</el-form-item>
				</el-col>
			</el-row>
	...
	export default {
		...
		methods: {
			...
			async getCategoryList(){
				var res = await this.$API.platform.store.category.tree.get();
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						let res;
						if (this.form.id) {
							res = await this.$API.platform.store.brand.update.put(this.form)
						} else {
							res = await this.$API.platform.store.brand.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.store.brand.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
