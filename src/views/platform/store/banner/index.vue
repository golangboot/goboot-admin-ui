		...
		<el-container class="is-vertical">
			...
			<el-main class="nopadding">
				<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
					...
					<el-table-column label="轮播图标题" prop="title" width="200" :show-overflow-tooltip="true"></el-table-column>
					...
					<el-table-column label="轮播图位置" prop="code" width="150" :show-overflow-tooltip="true"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.store.banner.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.banner.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.banner.update.put(reqData);
