	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="服务保障名称" prop="name" width="150"></el-table-column>
				<el-table-column label="服务保障描述" prop="description" width="300">
...
<script>
	...
	export default {
		name: 'platformStoreServiceGuarantee',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.platform.store.serviceGuarantee.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.serviceGuarantee.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.serviceGuarantee.update.put(reqData);
