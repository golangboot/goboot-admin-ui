...
<script>
	...
	export default {
		name: 'platformStoreDeviseCategory',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.platform.store.deviseCategory.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.deviseCategory.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.deviseCategory.update.put(reqData);
