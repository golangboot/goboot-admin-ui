	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.store.productComment.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.productComment.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.productComment.update.put(reqData);
