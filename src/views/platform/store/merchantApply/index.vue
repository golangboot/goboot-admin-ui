...
<script>
	...
	export default {
		name: 'platformStoreMerchantApply',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.platform.store.merchantApply.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.store.merchantApply.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.store.merchantApply.update.put(reqData);
