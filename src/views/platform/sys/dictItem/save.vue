		...
		data() {
          ...
          value: [
            {required: true, message: '请输入字典项值'}
				...
				dictSelect: {
					// api接口
					apiObj: this.$API.platform.sys.dict.list,
					// 参数(搜索关键字为空时生效)
					params: {},
					// 搜索参数(搜索关键字不为空时生效)
					search: {},
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						var res;
						if (this.form.id) {
							res = await this.$API.platform.sys.dictItem.update.put(this.form)
						} else {
							res = await this.$API.platform.sys.dictItem.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.sys.dictItem.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
