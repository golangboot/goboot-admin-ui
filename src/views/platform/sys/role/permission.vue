	...
	export default {
		...
		methods: {
			...
			async submit(){
				...
				let resPermission = await this.$API.platform.sys.role.assignPermission.post(reqData)
				let resMenu = await this.$API.platform.sys.role.assignMenu.post(reqData)
			...
			async getPermission(){
				...
				var res = await this.$API.platform.sys.permission.tree.get()
				this.permission.list = res.data

				let reqData = {id: this.form.id}
				let resPermission = await this.$API.platform.sys.role.getPermissionIdListByRoleId.get(reqData)
			...
			async getMenu(){
				...
				let reqData = {id: this.form.id}
				let resMenu = await this.$API.platform.sys.role.getMenuIdListByRoleId.get(reqData)
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.sys.role.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
			}
