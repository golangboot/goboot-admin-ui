	...
	<el-container>
		<el-header>
			<div class="left-panel">
				...
				<el-button type="primary" plain @click="syncRole">同步系统内置角色</el-button>
			</div>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.sys.role.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.sys.role.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.sys.role.update.put(reqData);
			...
			async syncRole(){
				var reqData = {}
				var res = await this.$API.platform.sys.role.sync.post(reqData);
				if(res.code == 200){
					this.$refs.table.refresh()
					this.$message.success("操作成功")
				}else{
