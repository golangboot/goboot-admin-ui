	...
	<el-dialog :title="titleMap[mode]" v-model="visible" :width="'80%'" :top="'5vh'" destroy-on-close @closed="$emit('closed')">
      ...
      <el-form-item label="租户域名" prop="domain">
        <template #label="{ label }">
          ...
          <span>
						<el-tooltip>
							<template #content>租户域名，一般为二级域名</template>
        ...
        <div class="el-form-item-msg">例如输入：<code>http://vip.example.com</code>，其中 <code>vip</code> 为租户二级域名，<code>example.com</code> 为平台顶级域名</div>
      </el-form-item>
			<el-row :gutter="20">
				<el-col :span="12">
        ...
        <div class="el-form-item-msg">租户授权使用时间，留空时表示永久有效</div>
      </el-form-item>
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						let res;
						if (this.form.id) {
							res = await this.$API.platform.sys.tenant.update.put(this.form)
						} else {
							res = await this.$API.platform.sys.tenant.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.platform.sys.tenant.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
			},
