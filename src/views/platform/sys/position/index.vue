	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.sys.position.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.sys.position.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.sys.position.update.put(reqData);
