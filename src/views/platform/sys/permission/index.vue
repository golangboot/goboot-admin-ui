	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.platform.sys.permission.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.platform.sys.permission.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.platform.sys.permission.update.put(reqData);
			...
			async syncPermission(){
				var reqData = {}
				var res = await this.$API.platform.sys.permission.sync.post(reqData);
				if(res.code == 200){
					this.$refs.table.refresh()
					this.$message.success("操作成功")
					// 触发树更新
			...
			//加载树数据
			async getTreeList(){
				let res = await this.$API.platform.sys.permission.tree.get();
			...
			async treeNodeDrop(draggingNode, dropNode, dropType) {
				treeUtils.treeNodeDrop(draggingNode, dropNode, dropType, async data => {
					this.loading = true;
					let res = data.id
						? await this.$API.platform.sys.permission.update.put(data)
						: await this.$API.platform.sys.permission.add.post(data);
					if (res.code == 200) {
