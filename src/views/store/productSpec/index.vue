		...
		<el-container class="is-vertical">
			...
			<el-main class="nopadding">
				<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
					...
					<el-table-column label="商品规格名称" prop="name" width="150"></el-table-column>
	...
	export default {
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.store.productSpec.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.store.productSpec.update.put(reqData);
