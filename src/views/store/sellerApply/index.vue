	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="卖家入驻申请名称" prop="name" width="150"></el-table-column>
				<el-table-column label="卖家入驻申请简介" prop="description" width="150" :show-overflow-tooltip="true"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.store.sellerApply.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.store.sellerApply.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.store.sellerApply.update.put(reqData);
