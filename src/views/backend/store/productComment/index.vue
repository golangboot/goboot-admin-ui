...
<script>
	...
	export default {
		name: 'backendStoreProductComment',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.backend.store.productComment.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.productComment.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.productComment.update.put(reqData);
