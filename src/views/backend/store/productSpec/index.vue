	...
	export default {
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.productSpec.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.productSpec.update.put(reqData);
