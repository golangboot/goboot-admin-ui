	...
	<el-dialog :title="titleMap[mode]" v-model="visible" :width="'80%'" :top="'5vh'" destroy-on-close @closed="$emit('closed')">
		<el-form :model="form" :rules="rules" :disabled="mode=='show'" ref="dialogForm" label-width="130px" label-position="right">
			...
			<el-form-item label="是否包邮" prop="isFreeShipping">
				...
				<div class="el-form-item-msg" v-if="form.isFreeShipping == 1">当前设置为包邮，将由商家承担运费</div>
			</el-form-item>
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						let res;
						if (this.form.id) {
							res = await this.$API.backend.store.shippingTemplate.update.put(this.form)
						} else {
							res = await this.$API.backend.store.shippingTemplate.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.backend.store.shippingTemplate.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
