...
<script>
	...
	export default {
		name: 'backendStoreShippingTemplate',
		components: {
			saveDialog,
			selectRemote,
		},
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.shippingTemplate.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.shippingTemplate.update.put(reqData);
