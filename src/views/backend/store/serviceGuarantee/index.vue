...
<script>
	...
	export default {
		name: 'backendStoreServiceGuarantee',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.backend.store.serviceGuarantee.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.serviceGuarantee.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.serviceGuarantee.update.put(reqData);
