        ...
        <el-col :span="12">
          <el-form-item label="经营类目" prop="merchantCategoryId">
            <select-remote v-model="form.merchantCategoryId"
                           :apiObj="$API.backend.store.merchantCategory.list"
        ...
        <el-col :span="12">
          <el-form-item label="店铺类型" prop="merchantTypeId">
            <select-remote v-model="form.merchantTypeId"
                           :apiObj="$API.backend.store.merchantType.list"
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						let res;
						if (this.form.id) {
							res = await this.$API.backend.store.merchantApply.update.put(this.form)
						} else {
							res = await this.$API.backend.store.merchantApply.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.backend.store.merchantApply.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
			}
