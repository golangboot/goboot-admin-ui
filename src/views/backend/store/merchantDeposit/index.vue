...
<script>
	...
	export default {
		name: 'backendStoreMerchantDeposit',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.backend.store.merchantDeposit.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.merchantDeposit.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.merchantDeposit.update.put(reqData);
