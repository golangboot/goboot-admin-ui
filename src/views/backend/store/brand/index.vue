		...
		<el-container class="is-vertical">
			...
			<el-main class="nopadding">
				<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
					...
					<el-table-column label="品牌图片" prop="image" fixed width="80">
	...
	export default {
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.brand.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.brand.update.put(reqData);
