			...
			<el-form :inline="true" :model="search" class="form-inline" style="vertical-align: middle;">
				...
				<el-form-item label="品牌：" prop="brandId">
					<select-remote v-model="search.brandId"
								   :apiObj="$API.backend.store.brand.list"
				...
				<el-form-item label="商家：" prop="merchantId">
					<select-remote v-model="search.merchantId"
								   :apiObj="$API.backend.store.merchant.list"
								   :params="{id: search.merchantId}"
								   :searchClearParams="['id']"
								   :request="{name: 'keyword'}"
								   :props="{label: 'name', value: 'id',}"
								   clearable filterable style="width: 160px;">
...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.backend.store.product.list,
			params: {},
			selection: [],
			search: {
				keyword: null,
	...
	methods: {
		...
		async table_del(row) {
			...
			var res = await this.$API.backend.store.product.delete.delete(reqData);
			if (res.code == 200) {
		...
		async changeSwitch(val, row) {
			...
			var res = await this.$API.backend.store.product.update.put(reqData);
