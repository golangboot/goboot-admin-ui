	...
	<el-drawer :title="titleMap[mode]" v-model="visible" :size="'96%'" :close-on-click-modal="mode == 'show'"
		...
		<el-container v-loading="loading">
			<el-main style="padding:0 20px 20px 20px;">
				<el-form :model="form" :rules="rules" :disabled="mode == 'show'" ref="form" label-width="130px"
					...
					<el-row :gutter="20" v-if="stepFormActive === 0">
						<el-col :span="24">
							<el-form-item label="商品分类" prop="categoryId">
								<el-cascader-panel v-model="form.categoryId" :options="treeOptions" :props="treeProps"
									:show-all-levels="true" style="width:100%" placeholder="请选择商品分类" clearable
						...
						<el-col :span="24">
							<el-card shadow="never" header="基础信息">
								...
								<el-form-item label="商品相册" prop="images">
									<sc-upload-multiple :width="80" :height="80" v-model="form.images" draggable :limit="10"
										title="请上传图片" tip="建议尺寸：800*800，可拖拽改变图片顺序，默认首张图为主图，最多上传10张"></sc-upload-multiple>
								</el-form-item>

								<el-row :gutter="20">
									<el-col :span="12">
										<el-form-item label="商品品牌" prop="brandId">
											<select-remote v-model="form.brandId" :apiObj="$API.backend.store.brand.list"
												:params="{ id: form.brandId, categoryId: form.categoryId }"
												:searchClearParams="['id']" :request="{ name: 'keyword' }"
												:props="{ label: 'name', value: 'id', }" clearable filterable
												style="width: 100%;">
											</select-remote>
										</el-form-item>
									</el-col>
									<el-col :span="12">
											...
											<template #label="{ label }">
												...
												<span>
													<el-tooltip>
														...
														<el-icon
															style="vertical-align: middle;margin-top: -3px;margin-left: 3px;"><el-icon-question-filled /></el-icon>
													</el-tooltip>
												</span>
											</template>
										...
										<!--<el-form-item label="销售单位" prop="saleUnitId">
											<select-remote v-model="form.saleUnitId"
														   :apiObj="$API.backend.store.saleUnit.list"
								...
								<el-form-item label="包装清单" prop="packingList">
									...
									<el-input v-model="form.packingList" :autosize="{ minRows: 2, maxRows: 4 }"
										:maxlength="255" :show-word-limit="true" type="textarea"></el-input>
								</el-form-item>
							...
							<el-card shadow="never" header="规格属性">
								<div ref="skuFormContainer">
									...
									<sku-form ref="skuForm" v-model:source-attributes="form.saleAttributes"
										v-model:attributes="form.skuAttributes" v-model:skus="form.skus"
										:structures="skuFormStructures" :table-props="skuFormTableProps"
										:sku-total-count-limit="100" :formDisabled="mode == 'show'">
										...
										<template #image="slotProps">
											<div class="image-upload-container" style="margin: 0 auto;">
												<sc-upload :width="80" :height="80" v-model="slotProps.row.image"
													title="请上传图片"></sc-upload>
											</div>
										</template>
										<template #status="slotProps">
											<div style="margin: 0 auto;">
												<el-switch v-model="slotProps.row.status" inline-prompt
													style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
													:active-value="1" :inactive-value="0" active-text="上架"
													inactive-text="下架" />
											</div>
										</template>
										...
										<template #operation="slotProps">
											<div class="operation-container" style="margin: 0 auto;">
												<el-popconfirm title="确定删除吗？"
													@confirm="skuFormTableRowDelete(slotProps.row, slotProps.index)">
													<template #reference>
							...
							<el-card shadow="never" header="规格参数"
								v-if="form.specAttributes && form.specAttributes.length > 0">
								<sc-form-table ref="specAttributesFormTable" v-model="form.specAttributes"
									:addTemplate="specAttributesAddTemplate" placeholder="暂无数据" hideDelete hideAdd>
										...
										<template #default="scope">
											...
											<el-form-item :key="`specAttributes-param-${scope.$index}`" label=" " label-width="30px"
												...
												<template v-if="scope.row?.selectType == 1 || scope.row?.selectType == 2">
													<el-select v-model="scope.row.param" placeholder=""
														style="width: 100%;">
												...
												<template v-else-if="scope.row?.selectType == 4">
													<el-switch v-model="scope.row.param" inline-prompt
														style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
														:active-value="'是'" :inactive-value="'否'" active-text="是"
														inactive-text="否" />
												</template>
												<template v-else>
							...
							<el-card shadow="never" header="商品详情">
								<el-form-item label="商品内容" prop="content">
									<sc-editor v-model="form.content" :disabled="mode == 'show'" placeholder="请输入内容"
										:height="500"></sc-editor>
								</el-form-item>
							</el-card>
										...
										<el-col :span="12">
											<el-form-item label="运费模板" prop="shippingTemplateId"
												...
												<select-remote v-model="form.shippingTemplateId"
													ref="shippingTemplateSelectRemote"
													:apiObj="$API.backend.store.shippingTemplate.list"
							...
							<el-card shadow="never" header="其他设置">
								<el-form-item label="商品关键词" prop="keywords">
									<template #label="{ label }">
										...
										<span>
											<el-tooltip>
												<template
													#content>商品关键词不影响搜索排序，请按照建议结构客观准确描述商品信息，避免带来商品转化或消费者投诉风险。</template>
												<el-icon
													style="vertical-align: middle;margin-top: -3px;margin-left: 3px;"><el-icon-question-filled /></el-icon>
											</el-tooltip>
										</span>
									</template>
									<el-input v-model="form.keywords" :autosize="{ minRows: 2, maxRows: 4 }"
										:maxlength="255" :show-word-limit="true" type="textarea"></el-input>
								</el-form-item>
								<el-form-item label="自定义表单状态" prop="customFormStatus">
									...
									<el-switch v-model="form.customFormStatus" class="form-switch" inline-prompt
										style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
								...
								<el-form-item label="自定义表单参数" prop="customFormParams" v-if="form.customFormStatus == 1">
									<sc-form-table ref="customFormTable" v-model="form.customFormParams"
										:addTemplate="customFormParamAddTemplate" placeholder="暂无数据" dragSort>
										...
										<el-table-column prop="type" label="类型">
											<template #default="scope">
												<el-select v-model="scope.row.type" placeholder="">
													<el-option v-for="(item, index) in customFormTypeOptions" :key="index"
														:label="item.label" :value="item.value" />
												</el-select>
											</template>
										</el-table-column>
										<el-table-column prop="required" label="是否必填">
											<template #default="scope">
												<el-switch v-model="scope.row.required" inline-prompt
													style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
													:active-value="true" :inactive-value="false" active-text="必填"
													inactive-text="选填" />
											</template>
										</el-table-column>
									</sc-form-table>
							...
							<el-card shadow="never" header="平台操作">
									...
									<el-col :span="12">
										<el-form-item label="商家" prop="merchantId">
											<select-remote v-model="form.merchantId"
												:apiObj="$API.backend.store.merchant.list"
												:params="{ id: form.merchantId }" :searchClearParams="['id']"
												:request="{ name: 'keyword' }" :props="{ label: 'name', value: 'id', }"
												clearable filterable style="width: 100%;">
											</select-remote>
										</el-form-item>
									</el-col>
								...
								<el-form-item label="审核状态" prop="auditStatus">
									<el-radio-group v-model="form.auditStatus">
										<el-radio v-for="(item, index) in auditStatusOptions" :key="index"
											:label="item.value">{{ item.label }}</el-radio>
									</el-radio-group>
								</el-form-item>
								<el-form-item label="备注" prop="remark">
									...
									<el-input v-model="form.remark" :autosize="{ minRows: 2, maxRows: 4 }" :maxlength="255"
								...
								<el-row :gutter="20">
									<el-col :span="12">
										<el-form-item label="虚拟销量" prop="virtualSaleCount">
											...
											<el-input-number v-model="form.virtualSaleCount" controls-position="right"
								...
								<el-row :gutter="20">
									<el-col :span="12">
										<el-form-item label="赠送积分" prop="giveIntegral">
											<el-input-number v-model="form.giveIntegral" placeholder="请输入赠送积分"
								...
								<el-alert title="以下参数为系统内部参数，一般由系统自动计算，如非必要，请勿改动！" type="warning" show-icon
								...
								<el-row :gutter="20">
									<el-col :span="12">
										<el-form-item label="销量" prop="saleCount">
											...
											<el-input-number v-model="form.saleCount" controls-position="right"
												style="width: 100%;"></el-input-number>
										</el-form-item>
									</el-col>
									<el-col :span="12">
										<el-form-item label="评价数" prop="commentCount">
											<el-input-number v-model="form.commentCount" placeholder=""
												controls-position="right" :min="0" style="width: 100%;"></el-input-number>
										</el-form-item>
									</el-col>
								</el-row>
								<el-row :gutter="20">
									<el-col :span="12">
										<el-form-item label="收藏数" prop="favoriteCount">
											<el-input-number v-model="form.favoriteCount" placeholder=""
												controls-position="right" :min="0" style="width: 100%;"></el-input-number>
										</el-form-item>
									</el-col>
								</el-row>
			...
			<el-footer>
				...
				<el-button v-if="mode != 'show' && stepFormActive == 1" type="primary" :loading="isSaving"
					@click="submit">保存</el-button>
				<el-button @click="visible = false">返回</el-button>
			</el-footer>
		</el-container>

		<shipping-template-save-dialog v-if="dialog.shippingTemplateSave" ref="shippingTemplateSaveDialog"
			@success="shippingTemplateHandleSaveSuccess"
			@closed="dialog.shippingTemplateSave = false"></shipping-template-save-dialog>
	</el-drawer>
</template>
...
export default {
	...
	data() {
		return {
			...
			brandSelect: {
				// api接口
				apiObj: this.$API.backend.store.brand.list,
				// 参数(搜索关键字为空时生效)
				params: {},
				// 搜索参数(搜索关键字不为空时生效)
				search: {},
			...
			saleUnitSelect: {
				// api接口
				apiObj: this.$API.backend.store.saleUnit.list,
				// 参数(搜索关键字为空时生效)
				params: {},
				// 搜索参数(搜索关键字不为空时生效)
				search: {},
			...
			merchantSelect: {
				// api接口
				apiObj: this.$API.backend.store.merchant.list,
				// 参数(搜索关键字为空时生效)
				params: {},
				// 搜索参数(搜索关键字不为空时生效)
				search: {},
			...
			shippingTemplateSelect: {
				// api接口
				apiObj: this.$API.backend.store.shippingTemplate.list,
				// 参数(搜索关键字为空时生效)
				params: {},
				// 搜索参数(搜索关键字不为空时生效)
				search: {},
	...
	methods: {
		...
		async submit() {
			...
			await this.$refs.form.validate(async (valid, val) => {
				if (valid) {
					...
					let res;
					if (this.form.id) {
						res = await this.$API.backend.store.product.update.put(this.form)
					} else {
						res = await this.$API.backend.store.product.add.post(this.form)
					}
					this.isSaving = false;
					if (res.code == 200) {
						this.form = res.data
		...
		async setData(data) {
			...
			this.$nextTick(async () => {
				if (data.id) {
					...
					let res = await this.$API.backend.store.product.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
				this.loading = false
		...
		async getTreeList() {
			let res = await this.$API.backend.store.category.tree.get();
			this.treeOptions = res.data
		},
		async getCategoryAttributes() {
			let reqData = { categoryId: this.form.categoryId }
			let res = await this.$API.backend.store.category.getCategoryAttributes.get(reqData);
