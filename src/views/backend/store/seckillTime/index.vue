	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.backend.store.seckillTime.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.seckillTime.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.seckillTime.update.put(reqData);
