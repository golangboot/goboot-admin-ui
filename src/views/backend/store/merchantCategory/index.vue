...
<script>
	...
	export default {
		name: 'backendStoreMerchantCategory',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.backend.store.merchantCategory.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.store.merchantCategory.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.store.merchantCategory.update.put(reqData);
