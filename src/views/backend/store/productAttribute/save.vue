		...
		<el-form :model="form" :rules="rules" :disabled="mode=='show'" ref="dialogForm" label-width="150px" label-position="right">
			...
			<el-form-item label="所属商品规格" prop="productSpecId">
				<select-remote v-model="form.productSpecId"
							   :apiObj="$API.backend.store.productSpec.list"
	...
	export default {
		...
		data() {
			return {
				...
				productSpecSelect: {
					// api接口
					apiObj: this.$API.backend.store.productSpec.list,
					// 搜索参数(搜索关键词为空时生效)
					params: {},
					// 属性字段
					props: {
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						let res;
						if (this.form.id) {
							res = await this.$API.backend.store.productAttribute.update.put(this.form)
						} else {
							res = await this.$API.backend.store.productAttribute.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.backend.store.productAttribute.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
