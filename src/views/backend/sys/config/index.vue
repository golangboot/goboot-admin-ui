	...
	export default {
		...
		data(){
			return {
				...
				apiObj: this.$API.backend.sys.config.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row, index){
				...
				var res = await this.$API.backend.sys.config.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.sys.config.update.put(reqData);
