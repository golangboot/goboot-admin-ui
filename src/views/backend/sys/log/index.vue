	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="用户ID" prop="userId" width="150"></el-table-column>
				...
				<el-table-column label="请求网址(URL)" prop="url" width="200" :show-overflow-tooltip="true"></el-table-column>
				...
				<el-table-column label="平台" prop="backend" width="100" :show-overflow-tooltip="true"></el-table-column>
	...
	export default {
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.sys.log.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.sys.log.update.put(reqData);
