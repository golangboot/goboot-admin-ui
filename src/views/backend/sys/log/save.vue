<template>
	<el-dialog :title="titleMap[mode]" v-model="visible" destroy-on-close @closed="$emit('closed')">
		<el-form :model="form" :rules="rules" :disabled="mode=='show'" ref="dialogForm" label-width="120px" label-position="right">
			...
			<el-row :gutter="20">
				<el-col :span="12">
					<el-form-item label="平台" prop="backend">
		...
		methods: {
			...
			submit(){
				this.$refs.dialogForm.validate(async (valid) => {
					if (valid) {
						...
						var res;
						if (this.form.id) {
							res = await this.$API.backend.sys.log.update.put(this.form)
						} else {
							res = await this.$API.backend.sys.log.add.post(this.form)
						}
						this.isSaving = false;
						if(res.code == 200){
							this.form = res.data
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.backend.sys.log.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
			}
