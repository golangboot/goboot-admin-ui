	...
	export default {
		...
		methods: {
			...
			async submit(){
				...
				let resPermission = await this.$API.backend.sys.role.assignPermission.post(reqData)
				let resMenu = await this.$API.backend.sys.role.assignMenu.post(reqData)
			...
			async getPermission(){
				...
				var res = await this.$API.backend.sys.permission.tree.get()
				this.permission.list = res.data

				let reqData = {id: this.form.id}
				let resPermission = await this.$API.backend.sys.role.getPermissionIdListByRoleId.get(reqData)
			...
			async getMenu(){
				...
				let res = await this.$API.backend.sys.menu.tree.get({groupId: 0})
				this.menu.list = res.data

				let reqData = {id: this.form.id}
				let resMenu = await this.$API.backend.sys.role.getMenuIdListByRoleId.get(reqData)
			...
			async setData(data){
				...
				if (data.id){
					...
					let res = await this.$API.backend.sys.role.detail.get(reqData)
					this.isSaving = false
					this.form = res.data
				}
			}
