	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.backend.sys.role.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.sys.role.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.sys.role.update.put(reqData);
			...
			async syncRole(){
				var reqData = {}
				var res = await this.$API.backend.sys.role.sync.post(reqData);
				if(res.code == 200){
					this.$refs.table.refresh()
					this.$message.success("操作成功")
				}else{
