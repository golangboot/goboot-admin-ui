				...
				<el-container class="is-vertical">
					...
					<el-main class="nopadding">
						<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
							...
							<el-table-column label="名称" prop="name" width="150" :show-overflow-tooltip="true"></el-table-column>
	...
	export default {
        ...
        this.$store.commit("getUserMenus")
			...
			async getTreeList(params = {}){
				let res = await this.$API.backend.sys.menu.tree.get(params);
			...
			async treeNodeDrop(draggingNode, dropNode, dropType) {
				treeUtils.treeNodeDrop(draggingNode, dropNode, dropType, async data => {
					this.loading = true;
					let res = data.id
						? await this.$API.backend.sys.menu.update.put(data)
						: await this.$API.backend.sys.menu.add.post(data);
					if (res.code == 200) {
