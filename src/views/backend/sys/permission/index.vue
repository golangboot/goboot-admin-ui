	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.backend.sys.permission.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.sys.permission.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.sys.permission.update.put(reqData);
			...
			async syncPermission(){
				var reqData = {}
				var res = await this.$API.backend.sys.permission.sync.post(reqData);
				if(res.code == 200){
					this.$refs.table.refresh()
					this.$message.success("操作成功")
					// 触发树更新
			...
			async treeNodeDrop(draggingNode, dropNode, dropType) {
				treeUtils.treeNodeDrop(draggingNode, dropNode, dropType, async data => {
					this.loading = true;
					let res = data.id
						? await this.$API.backend.sys.permission.update.put(data)
						: await this.$API.backend.sys.permission.add.post(data);
					if (res.code == 200) {
