	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" :params="params" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="文章标题" prop="title" width="200">
				...
				<el-table-column label="文章简介" prop="description" width="200">
					<template #default="scope">
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.backend.cms.article.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.cms.article.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.cms.article.update.put(reqData);
