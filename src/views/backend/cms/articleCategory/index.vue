...
<script>
	...
	export default {
		name: 'backendCmsArticleCategory',
		components: {
			saveDialog,
		},
		data() {
			return {
				...
				apiObj: this.$API.backend.cms.articleCategory.list,
				params: {},
				selection: [],
				search: {
					keyword: null
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.cms.articleCategory.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.backend.cms.articleCategory.update.put(reqData);
			...
			//加载树数据
			async getTreeList(){
				let res = await this.$API.backend.cms.articleCategory.tree.get();
			...
			async treeNodeDrop(draggingNode, dropNode, dropType) {
				treeUtils.treeNodeDrop(draggingNode, dropNode, dropType, async data => {
					this.loading = true;
					let res = data.id
						? await this.$API.backend.cms.articleCategory.update.put(data)
						: await this.$API.backend.cms.articleCategory.add.post(data);
					if (res.code == 200) {
