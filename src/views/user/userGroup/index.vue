	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="用户组编码" prop="code" width="200"></el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				apiObj: this.$API.user.group.list,
				selection: [],
				search: {
					keyword: null
				}
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.user.group.delete.delete(reqData);
				if(res.code == 200){
			...
			async changeSwitch(val, row){
				...
				var res = await this.$API.user.group.update.put(reqData);
