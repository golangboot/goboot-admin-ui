	...
	export default {
		...
		methods: {
			...
			async table_del(row){
				...
				var res = await this.$API.backend.sys.fileDetail.delete.delete(reqData);
				if(res.code == 200){
			...
			deleteSelect(){
				...
				this.$confirm(`确定删除选中的 ${ids.length} 项吗？`, '提示', {
					...
					let res = await this.$API.backend.sys.fileDetail.delete.delete(reqData)
					if (res.code != 200) {
			...
			//加载树数据
			async getTreeList(){
				let res = await this.$API.backend.sys.fileCategory.tree.get();
			...
			async treeNodeDrop(draggingNode, dropNode, dropType) {
				treeUtils.treeNodeDrop(draggingNode, dropNode, dropType, async data => {
					this.loading = true;
					let res = data.id
						? await this.$API.backend.sys.fileCategory.update.put(data)
						: await this.$API.backend.sys.fileCategory.add.post(data);
					if (res.code == 200) {
			...
			async treeDel(){
				...
				var res = await this.$API.backend.sys.fileCategory.delete.delete(reqData)
