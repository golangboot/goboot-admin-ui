...
export default {
	...
	pageSize: 20,													//表格每一页条数
	pageSizes: [10, 20, 30, 40, 50, 100, 500, 1000],				//表格可设置的一页条数
	...
	parseData: function (res) {										//数据分析
		return {
			...
			rows: res.data?.records || res.data?.rows,		//分析行数据字段结构
	...
	request: {							//请求规定字段
		page: 'current',					//规定当前分页字段 page
		pageSize: 'size',			//规定一页条数字段 pageSize
		prop: 'prop',					//规定排序字段名字段 prop
		order: 'order'					//规定排序规格字段 order
	},
	/**
	 * 自定义列保存处理
